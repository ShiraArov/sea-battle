...
class Screen:
    # class that represent one screen
    def __init__(self, img_name, left_table, right_table, locations_true, locations_false, surface, list_of_ships):
        self.img_name = img_name  # the name of the screen's image
        self.left_table = left_table  # true / false for the left table
        self.right_table = right_table  # true / false for the right table
        self.locations_true = locations_true  # hits list if exist else []
        self.locations_false = locations_false  # not hits list if exist else []
        self.surface = surface  # surface's name
        self.list_of_ships = list_of_ships  # list of ship's objects
    
    def show_screen(self):
        self.surface.set_colorkey((0, 0, 0))
        self.surface.blit(self.img_name, (0, 0))
        pygame.display.flip()

        # print the left black table
        if self.left_table:
            for i in range(20, 461, 40):
                pygame.draw.line(self.surface, BLACK, [i, 200], [i, 640], 2)
                pygame.draw.line(self.surface, BLACK, [20, 180 + i], [460, 180 + i], 2)
            pygame.display.flip()

        # print the right black table
        if self.right_table:
            for i in range(540, 981, 40):
                pygame.draw.line(self.surface, BLACK, [i, 200], [i, 640], 2)
                pygame.draw.line(self.surface, BLACK, [540, i - 340], [980, i - 340], 2)
            pygame.display.flip()

        # print the ships
        for obj in self.list_of_ships:
            obj.img_name.set_colorkey(WHITE)
            self.surface.blit(obj.img_name, obj.top_left)
        pygame.display.flip()

        if not self.locations_true == [] and not self.locations_false == []:
            # print v's in the location that got hit
            for location in self.locations_true:
                TRUE_LOCATION.set_colorkey(WHITE)
                self.surface.blit(TRUE_LOCATION, location)
            pygame.display.flip()

            # print x's in the location that got hit
            for location in self.locations_false:
                FALSE_LOCATION.set_colorkey(WHITE)
                self.surface.blit(FALSE_LOCATION, location)
            pygame.display.flip()
